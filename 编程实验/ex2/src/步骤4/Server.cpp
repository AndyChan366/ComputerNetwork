...
unsigned __stdcall socketRecv(void* p)
{
	...
	sprintf(ip_addr, "%d.%d.%d.%d", int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b1), int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b2),
		int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b3), int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b4));
	sprintf(sendbuf, "ip: %s port: %d\ntime: %smessage: %s", ip_addr, int(ClientAddr[id].sin_port), dt, "Enter!");
	printf("%s\n\n", sendbuf);
	for (int i = 0; i < DEFAULT_SOCKNUM; i++)
	{
		if (ClientSocket[i] != INVALID_SOCKET)
	...
	while (true)
	{
		...
		if (iResult > 0)
		{
			...
			sprintf(ip_addr, "%d.%d.%d.%d", int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b1), int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b2),
				int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b3), int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b4));
			sprintf(sendbuf, "ip: %s port: %d\ntime: %smessage: %s", ip_addr, int(ClientAddr[id].sin_port), dt, recvbuf);
			printf("%s\n\n", sendbuf);
	...
	sprintf(ip_addr, "%d.%d.%d.%d", int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b1), int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b2),
		int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b3), int(ClientAddr[id].sin_addr.S_un.S_un_b.s_b4));
	sprintf(sendbuf, "ip: %s port: %d\ntime: %smessage: %s", ip_addr, int(ClientAddr[id].sin_port), dt, "Leave!");
	printf("%s\n\n", sendbuf);
